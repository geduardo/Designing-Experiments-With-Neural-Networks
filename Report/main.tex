...
uniformly at random from the intervals $0.1$ kg $< M < 1$ kg and \break $ 0.1$
cm $<L<10 $ cm. The space of states of the SPE is then:
 \begin{equation}
...
We can therefore set a very easy decision rule for our experimenter
reinforcement learning agent:
\begin{itemize}
  \item Let $\textbf{Q}=(q_0,q_1)$ be the value table, where $q_0$ and $q_1$ are
  the values associated with the actions 0 and 1 respectively. The initial value
  of $q_0$ and $q_1$ is set to 0.
  \item  Every episode, the agent takes an action $a$ and receives a reward $r$
  after taking the action. The update rule for $\textbf{Q}$ is the following:
  \begin{equation}
    q_a\leftarrow q_a + \alpha r
  \end{equation}
  where $\alpha$ is the learning rate. We store in $q_a$ the cumulative reward
  obtained by the action $a$ weighted with a learning rate.
  \item The agent's policy or decision rule is:
  \begin{equation}
    a = \argmax_{x \in \mathcal{A}} \textbf{Q}
    \label{argmax_policy}
  \end{equation}
  This is, the Experimenter takes the action $a$ with the highest $q_a$.
\end{itemize}

This rule is still flawed, since the first action that gets a reward would
always be elected afterward independently of its optimality. To solve this we
can force the agent to initially explore different options ignoring the decision
rule, and slowly, when the values $q_a$ are more reliable, let the agent choose
according to \eqref{argmax_policy}. The easiest way to achieve that is with an
$\epsilon$-greedy decision algorithm: we set an exploration rate $\epsilon$ that
decreases in each episode until it reaches a minimum value
